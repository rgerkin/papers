...
executable functions, here written in Python, that generate predictions from a quantitative model and statistically validate these predictions against a relevant experimental observation to produce a score indicating agreement between the model and the data. 
Suites of such validation tests are collaboratively developed by a scientific community in common repositories  to produce up-to-date statistical summaries of the state-of-the-art in a scientific community. In this paper, we aim to detail this test-driven workflow and introduce it to the neuroscience community. To be concrete, we describe \textit{NeuronUnit}, a library that builds upon \textit{SciUnit} and integrates with several existing neuroinformatics resources to support validating single-neuron models using data gathered by neurophysiologists. 
