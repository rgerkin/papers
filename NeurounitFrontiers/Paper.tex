...
A simple \emph{family} of models, \verbx{LinearModel}, implementing the capability in Fig. \ref{fig:capability} as well as another capability shown in Fig. \ref{fig:training}, is shown in Fig. \ref{fig:simple_model}. 
...
An alternative test design to the one in Fig. \ref{fig:rate_test} would add a second required capability (Fig. \ref{fig:training}) and a corresponding training phase to the \verbx{_judge} method. 
...
Although Fig. \ref{fig:rate_test} does not include a training phase, if training data is available then models that nevertheless implement a training capability (like \verbx{LinearModel}) can simply be trained explicitly by calling the capability method just like any other Python method:
\begin{python}
