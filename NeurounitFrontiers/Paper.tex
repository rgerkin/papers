...
NeuronUnit can in principle test arbitrary neurophysiology models using arbitrary data but we provide here an example using models described in NeuroML as part of the Open Source Brain (OSB) project (\url{http://www.opensourcebrain.org}), and single neuron electrophysiology data available as part of the NeuroElectro Project (\url{http://neuroelectro.org}).  
...
Capabilities are simply collections of methods that a test may need to invoke to receive relevant data, and are analogous to \emph{interfaces} in e.g. Java (\url{http://docs.oracle.com/javase/tutorial/java/concepts/interface.html}). 
...
By using machine-readable models from Open Source Brain (OSB), and machine-readable data from resources like The NeuroElectro Project (\url{http://www.neuroelectro.org}), we have produced a suite of data-driven validation tests in \textit{NeuronUnit}, as well as tools for capability implementation, and model interfaces for a representative set of canonical neurophysiology models, all publicly available. 

\label{sec:neuronunit_acitivities}
Here we describe standard neuroinformatics tools we have adopted to develop \textit{NeuronUnit} (\url{http://github.com/scidash/neuronunit}) \todo{Must change name of repository on github to neuronunit (after I am sure grant has been reviewed)}.  
...
NeuroConstruct (\citep{gleeson_neuroconstruct:_2007}, \url{http://www.neuroconstruct.org}) is a simulation manager that takes NeuroML models and hands off simulation to supported simulators. 
...
The Open Source Brain project (OSB, \url{http://www.opensourcebrain.org}) curates many models described in NeuroML. 
OSB-curated projects are converted from their native format into NeuroML, and run on major neural simulators (\url{http://www.neuron.yale.edu/neuron}, \url{http://genesis-sim.org}, \url{http://www.nest-initiative.org}, \url{http://moose.ncbs.res.in}). 
...
We used a service called \textit{SciDash} (\url{http://www.scidash.org}) for coordinated development of software repositories.
...
The \textbf{first layer} is completely under the control of individual developer communities, and consists of git repositories \citep{ram_git_2013} stored on GitHub \url{http://github.com}. 
...
Automated generation of NeuroML from native model source code is in development (Gleeson, personal communication); for the popular NEURON simulator (\url{http://www.neuron.yale.edu/neuron}), this functionality is already mature and in use. 
...
Models can be sub- or supersets of other models; similarly for SBML (\citep{hucka_systems_2003}, \url{http://sbml.org}) a general systems biology markup language. 
...
identification and correction of flawed methodology is becoming increasingly common (\citep{button_power_2013,kriegeskorte_circular_2009,galbraith_study_2010}, \url{http://prefrontal.org/files/posters/Bennett-Salmon-2009.jpg}), and is accelerated by an open framework. 
...
SBML (\citep{hucka_systems_2003}, \url{http://sbml.org} will enable expansion beyond neuroscience, facilitated by parallel efforts among NeuroML developers to interface with it (Crook, unpublished). 
