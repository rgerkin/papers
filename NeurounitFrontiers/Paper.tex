...
\end{python}
A simple summary of the scores in a score matrix can be printed to the console or visualized by other tools in development.  For example, the score matrix might be visualized as a hyperlinked table inside an IPython notebook \todo{reference to ipython nb docs/paper?} containing the specification and execution of the test suite (Fig. \ref{fig:scidash_matrix}). Such notebooks are stored in test repositories like \textit{hippocampus\_tests}.
...
For example, scientists interested specifically in testing models of cells in the hippocampus would use \textit{NeuronUnit} to create tests in a GitHub repository containing an iPython notebook, as described above, called \textit{hippocampus\_tests}, parameterizing them by relevant data from \textit{NeuroElectro}. 
...
Once the testing logic has been changed, all past models will be immediately evaluated against the new metrics. 
