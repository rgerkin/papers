...
Neuroscientists construct quantitative models to coherently explain a body of observations about neurons, circuits, brain regions and behavior. 
...
Today, modelers contribute a new model to the research community by submitting a paper describing how it works, along with selected figures demonstrating its scope and validity and comparing its validity to other models with the same scope. Reviewers are then responsible for discovering relevant data and competing models that the paper did not adequately consider, relying on their knowledge of the publication history of the field. Unfortunately, in many research areas, the volume of research being published every year is outpacing the abilities of even the most SOMETHING \todo{need good adjective} scientists \todo{citation/stats?}. 
...
Readers, like reviewers, face similar challenges when attempting to answer fundamental questions like these, given the enormity of the publication history in many fields:
\begin{itemize}
...
\item How well do these models perform, according to these metrics?
\item What other quantities can these models predict?
\item What observations are not adequately explained by available models?
\end{itemize}
...
In professional software development, complex programs are not validated by simply choosing a few interesting inputs and presenting the outputs of the program on these inputs to reviewers. Instead, developers typically follow a \emph{test-driven development} methodology by creating a suite of executable \emph{unit tests} that collectively validate the functionality of a program as it is being developed and modified. Each test is responsible for checking that a small portion of the program meets a single correctness criterion. For example, a unit test might verify that one function within the program correctly handles malformed inputs. Collectively, the test results serve as summary of the validity of the software as it progresses through its development cycle. Developers can determine which features are unimplemented or buggy by examining the set of failed tests, and progress can be measured in terms of how many tests the program passes over time. This methodology is widely adopted and has been highly successful in practice \cite{tdd}\todo{cite tdd}.
