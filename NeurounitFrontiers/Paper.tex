...
We implement functions bridging this infrastructure with \textit{SciUnit}, as a collection of relevant model and test families and associated capabilities, in a package called \textit{NeuronUnit}. 
This package is collaboratively developed on Github (\url{http://github.com/scidash/neuronunit})\todo{Must change name of repository on github to neuronunit (after I am sure grant has been reviewed)}. 
Scientists can use these tools to easily create test suites tailored to specific systems of interest. 
For example, scientists interested specifically in testing models of cells in the hippocampus would use \textit{NeuronUnit} to create tests in a Github repository called \textit{hippocampus_tests}, parameterized by relevant data from \textit{NeuroElectro}. 
To submit a new model for testing, a scientist can fork this repository, add their models, test it locally, and when satisfied, submit a pull request to the common repository. 
Each community can develop suitable quality standards for merging pull requests. 
Similarly, statisticians interested in improving the methods by which goodness-of-fit are tested can follow the same procedure, rather than attempting to convince scientists socially that the methods used in canonical papers need to be changed.
\todo{Sentence about how great github is for collaboratively doing things, particularly in science?}
