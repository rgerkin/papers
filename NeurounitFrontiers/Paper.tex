...
executable functions, here written in Python, that generate and statistically validate predictions from a specified class of scientific models against one relevant empirical observation to produce a score indicating agreement between the model and the data. 
...
\tiny
 \keyFont{ \section{Keywords:} Neuroinformatics Simulation Electrophysiology Software Modeling Validation\todo{is this the right format for these?} } %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
...
Professional software developers face similar issues\todo{probably cite our in submission paper here??}. 
...
Recognizing this, and inspired by unit testing practices, we have developed a discipline-agnostic framework for developing \emph{model validation test suites} called \textit{SciUnit} (available from \url{http://sciunit.scidash.org}) \todo{cite sciunit paper again here?}. 
...
\end{python}\todo{decrease vertical space after python}
...
\end{python}
A simple summary of the scores in a score matrix can be printed to the console or visualized by other tools in development\todo{mention use within iPython to generate an HTML table}.
...
In particular, we will continue to focus on neurophysiology as our initial case study, using machine-readable models written in \textit{NeuroML} available from OpenSourceBrain (\textit{OSB}), and machine-readable data from \textit{NeuroElectro}. We implement functions bridging this infrastructure with \textit{SciUnit}, as a collection of relevant model and test families and associated capabilities, in a package called \textit{NeuronUnit}. This package is collaboratively developed on Github (\url{http://github.com/scidash/neuronunit})\todo{Must change name of repository on github to neuronunit (after I am sure grant has been reviewed)}. Scientists can use these tools to easily create test suites tailored to specific systems of interest. For example, scientists interested specifically in testing models of cells in the hippocampus would use \textit{NeuronUnit} to create tests in a Github repository called \textit{HippocampusTests},  parameterized by relevant data from \textit{NeuroElectro}. To submit a new model for testing, a scientist can fork this repository, add their models, test it locally, and when satisfied, submit a pull request to the common repository. Each community can develop suitable quality standards for merging pull requests. Similarly, statisticians interested in improving the methods by which goodness-of-fit are tested can follow the same procedure, rather than attempting to convince scientists socially that the methods used in canonical papers need to be changed.\todo{Sentence about how great github is for collaboratively doing things, particularly in science?}

We anticipate that other communities within neuroscience (and indeed, within science more broadly) will develop repositories similar to \textit{NeuronUnit} that provide a common, collaboratively-developed vocabulary of test families, model families and capabilities relevant to their domain, and bridge with relevant standardization efforts and infrastructure. Using these common repositories, small groups of scientists interested in particular systems of interest will develop tests and curate models in test repositories like \textit{HippocampusTests}. The end result of these efforts will be tables, like the one shown in Figure \ref{fig:sciunit_overview}, that help scientists understand, precisely, the state-of-the-art in their field.
